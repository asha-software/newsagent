...
from .utils import (
    ...
    initialize_builtin_tools_for_user,
)
import json
import requests
...
def get_user_api_key(request, redirect_url=None, redirect_kwargs=None):
    ...
    if not api_key:
        messages.error(
            request, "No active API key found. Please create an API key first."
        )
        if redirect_kwargs:
                ...
                if settings.EMAIL_VERIFICATION_ENABLED:
                    ...
                    try:
                        ...
                        email_sent = send_verification_email(
                            user_obj, verification.token, request
                        )
                        if email_sent:
                            return render(
                                request,
    ...
    if request.method == "POST":
        ...
        if pending_username:
            return render(
                ...
                "signup.html",
                {
                    "error": "There is already a pending registration for this username. Please check your email for the verification link."
                },
            )
        ...
        if pending_email:
            return render(
                ...
                "signup.html",
                {
                    "error": "There is already a pending registration for this email. Please check your email for the verification link."
                },
            )
        ...
        if settings.EMAIL_VERIFICATION_ENABLED:
            ...
            try:
                ...
                return render(
                    request, "signup.html", {"error": f"Registration failed: {str(e)}"}
                )
        else:
...
def verify_email(request, token):
    ...
    try:
        ...
        if pending.is_expired():
            ...
            return render(
                request,
                "user_info/email_verification_success.html",
                {
                    "success": False,
        ...
        try:
            ...
            return render(
                request,
                "user_info/email_verification_success.html",
                {"success": False, "error_message": "Invalid verification link."},
            )
...
def search(request):
    ...
    if request.method == "POST":
        ...
        if query:
            ...
            custom_tools = UserTool.objects.filter(user=request.user).exclude(
            ...
            if existing_result:
                ...
                serialized_result_data = json.dumps(
                    json.dumps(existing_result.result_data)
                )
    ...
    if (
        ...
        return JsonResponse({"success": True, "has_cached_result": False})
    ...
    user_tools = (
        UserTool.objects.filter(user=request.user, is_active=True)
        .exclude(name__in=builtin_tool_names)
        .order_by("name")
    )
    ...
    for tool_name in builtin_tool_names:
        ...
        tool_preference = UserTool.objects.filter(
            user=request.user, name=tool_name
        ).first()
...
def shared_search_result(request, result_id):
    try:
        ...
        if not shared_result.is_public and (
            not request.user.is_authenticated or request.user != shared_result.user
        ):
...
def save_shared_result(request):
    if request.method == "POST":
        try:
            ...
            return JsonResponse(
                {"success": False, "message": f"Error saving result: {str(e)}"},
                status=400,
            )
            ...
            if settings.EMAIL_VERIFICATION_ENABLED:
                ...
                try:
                    email_sent = send_password_reset_email(
                        user, reset_token.token, request
                    )
...
def reset_password_view(request, token):
    ...
    try:
        ...
        if reset_token.is_expired() or reset_token.is_used:
            return render(request, "user_info/reset_password.html", {"expired": True})
        ...
        return render(request, "user_info/reset_password.html", {"token": token})
...
def tool_list(request):
    ...
    tools = (
        UserTool.objects.filter(user=request.user)
        .exclude(name__in=builtin_tool_names)
        .order_by("-created_at")
    )
...
def get_api_key(request):
    ...
    return JsonResponse({"api_key": api_key.key, "username": request.user.username})
...
def tool_create(request):
    if request.method == "POST":
        ...
        if form.is_valid():
            ...
            if success:
                messages.success(
                    request, f"Tool '{tool_data['name']}' created successfully!"
                )
                ...
                return render(
                    request,
                    "user_info/tool_form.html",
                    {"form": form, "title": "Create New Tool"},
                )
    else:
        form = UserToolForm()

    return render(
        request, "user_info/tool_form.html", {"form": form, "title": "Create New Tool"}
    )
...
def tool_edit(request, tool_id):
    ...
    if request.method == "POST":
        ...
        if form.is_valid():
            # Get the user's API key
            api_key, error_response = get_user_api_key(
                request, "tool_edit", {"tool_id": tool_id}
            )
            if error_response:
                return error_response
            ...
            if not success:
                ...
                return render(
                    request,
                    "user_info/tool_form.html",
                    {"form": form, "tool": tool, "title": f"Edit Tool: {tool.name}"},
                )
            ...
            if success:
                messages.success(
                    request, f"Tool '{tool_data['name']}' updated successfully!"
                )
                return redirect("tool_list")
            else:
                # Show the error message on the form page
        ...
        initial_data = {
            field: json.dumps(getattr(tool, field), indent=2)
            if getattr(tool, field)
            else ""
            for field in [
                "headers",
                "default_params",
...
def history(request):
    ...
    user_queries = SharedSearchResult.objects.filter(user=request.user).order_by(
        "-created_at"
    )
...
def apikey_create(request):
    if request.method == "POST":
        ...
        if existing_keys_count >= 3:
            messages.error(
                request, "You can only have a maximum of 3 API keys per account."
            )
