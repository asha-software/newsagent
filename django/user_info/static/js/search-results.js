  ...
  if (resultsContainer) {
    ...
    if (data.analyses && Array.isArray(data.analyses)) {
      ...
      if (data.final_label && data.final_justification) {
        ...
        finalVerdictIcon.src = data.final_label === 'true' ? YES_ICON_URL : NO_ICON_URL;
        ...
        finalJustificationValue.textContent =
          data.final_justification || 'No final justification provided';
      ...
      for (let i = 0; i < data.analyses.length; i++) {
        ...
        verdictIcon.src = analysis.label === 'true' ? YES_ICON_URL : NO_ICON_URL;
      ...
      data.justifications &&
      Array.isArray(data.justifications)
    ) {
      ...
      for (let i = 0; i < data.claims.length; i++) {
        ...
        const verdictIcon = document.createElement('img');
        verdictIcon.src = data.labels[i] === 'true' ? YES_ICON_URL : NO_ICON_URL;
      ...
      if (data.final_label && data.final_justification) {
        ...
        finalVerdictJustification.textContent =
          data.final_justification || 'No final justification provided';
      ...
      data.forEach(result => {
        ...
        icon.src = result.label === 'true' ? YES_ICON_URL : NO_ICON_URL;
...
if (isSharedView) {
  ...
  if (sharedResultData) {
    try {
      ...
      resultsContainer.innerHTML =
        '<div class="error">Error displaying shared result: ' + error.message + '</div>';
      resultsContainer.style.display = 'block';
    }
  }
}
