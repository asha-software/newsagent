...
def create_analyses(
    ...
    return [
        ...
        for claim, label, justification, evidence_list in zip(
            claims, labels, justifications, evidence_lists
        )
    ]


async def process_query(
    text: str, builtin_tools: list, user_tool_kwargs: list = []
) -> dict:
    ...
    result = claim_decomposer.invoke(
        initial_state,
        config={
            "run_name": "claim_decomposer",
        },
    )
    claims = result["claims"]

    research_results = [
        research_agent.invoke({"claim": claim}, config={"run_name": "research_agent"})
        for claim in claims
    ]
    delete_messages(research_results)

    reasoning_results = [
        reasoning_agent.invoke(state, config={"run_name": "reasoning_agent"})
    ...
    analyses = create_analyses(
        verdict_results["claims"],
        verdict_results["labels"],
        verdict_results["justifications"],
        verdict_results["evidence"],
    )
