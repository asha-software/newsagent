...
def register_method(
    ...
    tool_instance.register_tool_function(
        method_info["input_types"], method_function, method_info["description"]
    )
    ...
    globals()[method_name] = langchain_tool(description=method_info["description"])(
        method_function
    )
...
def construct_url(tool_dict):
    ...
    if isinstance(tool_dict, str):
        try:
            ...
            raise ValueError(
                "construct_url expects a dictionary or a valid JSON string!"
            )
