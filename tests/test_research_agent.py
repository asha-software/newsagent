...
def test_agent_processes_claim():
    ...
    print()
    assert (
        len(final_state["messages"]) >= 3
    ), "Final state should have at least 3 messages: System, Human (claim) and AI (research agent response to claim)"
    assert len(final_state["evidence"]) > 0, "Final state should contain evidence"
...
def test_tool_use(test_case):
    ...
    final_state = agent.invoke({"claim": claim, "messages": [], "evidence": []})
    ...
    formatted_description = description.format(
        expected_tools=expected_tools, tools_used=tools_used
    )
