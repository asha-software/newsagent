...
def argument_parser():
    parser = argparse.ArgumentParser(description="Evaluate the claim decomposer")
    parser.add_argument(
...
def target_function(initial_state):
    ...
    assert (
        "RESEARCH_AGENT_MODEL" in os.environ
    ), "Please set the RESEARCH_AGENT_MODEL environment variable"
    ...
    agent = create_agent(
        model=model, builtin_tools=["calculator", "wikipedia", "web_search"]
    )
    results = agent.invoke(initial_state)
    return results["evidence"]


def evaluate_tool_choices(
    outputs: list[Evidence], reference_outputs: list[Evidence]
) -> bool:
    # need to check that the correct tool was invoked
    ref_toolset = set([ref["name"] for ref in reference_outputs["evidence"]])
    output_toolset = set([tool_call["name"] for tool_call in outputs["output"]])
...
def main():
    ...
    ), "Please set the LANGCHAIN_API_KEY environment variable"
    assert (
        os.environ["LANGCHAIN_TRACING_V2"] == "true"
    ), "Please set the LANGCHAIN_TRACING_V2 environment variable to true"
