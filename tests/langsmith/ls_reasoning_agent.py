...
def target_function(inputs) -> dict:
    ...
    return {
        "output": {"label": result["label"], "justification": result["justification"]}
    }
...
def justification_coherence(outputs: dict, reference_outputs: dict) -> dict:
    ...
    prompt = prompt_template.format(
        predicted_label=predicted_label, justification=justification
    )
...
def f1_score_summary_evaluator(
    ...
    true_positives = sum(
        ...
        if output["output"]["label"] == reference["label"]
    )
    false_positives = sum(
        1
        for output, reference in zip(outputs, reference_outputs)
    ...
    false_negatives = sum(
        1
        for output, reference in zip(outputs, reference_outputs)
        if reference["label"] != output["output"]["label"]
    )
...
def main():
    ...
    ), "Please set LANGCHAIN_TRACING_V2 environment variable to true"
